...
function sendFileToDS (pData) {
	...
	var contentType = { "Content-Type" : "multipart/form-data;charset=latin1;boundary=" + boundary};
	var postData = {"api":"SYNO.DownloadStation.Task","version":"1","method":"create"};
	var multipart = "";
	...
	multipart += "------" + boundary
                   + "\r\nContent-Disposition: form-data; name=\"file\" ;filename=\"fromfirefox.torrent\""
	...
	var addTask = Request({
		...
		content: multipart,
		headers:contentType,
		overrideMimeType: "charset=latin1",
		onComplete: function (response) {
			console.log("Get response ...");
			var myconnect = response.json;
			if (response.status != "200") {
